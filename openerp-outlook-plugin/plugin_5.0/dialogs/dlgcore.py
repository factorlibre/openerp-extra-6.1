...
class ProcessorDialog(TooltipDialog):
            ...
            for int_id in self.command_processors:
                try:
                    self.GetDlgItem(int_id)
                     ...
                     logger.notifyChannel("ERROR: Dialog item %s refers to an invalid control" % self._GetIDName(int_id))
    ...
    def GetPopupHelpText(self, iCtrlId):
        ...
        if cp is not None:
         ...
         logger.notifyChannel("Can not get command processor for", self._GetIDName(iCtrlId))
        return None
    def OnRButtonUp(self, hwnd, msg, wparam, lparam):
