...
<blockTable style="mytable">
 <tr><td>
    <para style="nospace"><xsl:text> </xsl:text><b><xsl:value-of select="object.name"/></b></para>
    <para style="nospace"><xsl:text> </xsl:text><xsl:value-of select="object.ean13"/></para>
 </td></tr>
</blockTable>
<nextFrame/>
</xsl:template>
