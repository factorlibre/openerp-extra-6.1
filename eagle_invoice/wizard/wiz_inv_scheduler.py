...
class wizard_setup_invoice_scheduler(osv.osv_memory):
	_name = 'eagle.setup_inv_sched'
	_description = 'Setup an Eagle Invoice Scheduler'
	...
	def setup_scheduler(self, cr, uid, ids, context=None):
		...
		cron_ids = crons.search( cr, uid, [('model','=','eagle.contract'),('function','=','run_inv_scheduler')], context=context )
		if not cron_ids or len(cron_ids) < 1:
			vals = {
				'model': 'eagle.contract',
				'function': 'run_inv_scheduler',
				'name': 'Eagle Invoices Scheduler',
				'interval_number': 1,
				'interval_type': 'days',
				'numbercall':-1,
...
wizard_setup_invoice_scheduler()
